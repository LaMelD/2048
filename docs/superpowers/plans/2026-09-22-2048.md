...
`app.json`을 아래 내용으로 **교체**한다. `expo.name`은 스토어 표시명과 별개로 앱 내부 이름이다.
...
`assets/` 안의 파일명은 create-expo-app이 생성한 실제 파일명과 맞춰야 한다. Step 2에서 `ls assets/`로 확인하고 다르면 그 이름을 쓴다.

- [ ] **Step 5: .gitignore에 키스토어 관련 항목 추가**

`.gitignore` 끝에 덧붙인다. `android/`는 **추가하지 않는다** — 릴리즈 서명 설정이 그 안에 살아야 하므로 커밋한다.
